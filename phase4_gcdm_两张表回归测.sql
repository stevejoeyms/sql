...
--dwc_dim_cus_gcdm_customer_address_t clean	--08.20修改
select
	...
	case when trim(postalcode) rlike '^[0-9]{6}$' then trim(postalcode) end	postcode,	--08.20修改
	preferred	preferred,
...
--dwc_dim_cus_gcdm_customer_communication_t clean	--08.20修改
select 
	communication_status
	,communication_type
	,cons_guid
	,case 
		when communication_type in ('MOB','TEL') then value
		when communication_type='EMAIL' and length(REGEXP_REPLACE(value,'[^@]',''))=0 or length(REGEXP_REPLACE(value,'[^.]',''))=0 then null
		when communication_type='EMAIL' and length(REGEXP_REPLACE(substr(value,instr(value,'@')),'[^.]',''))=0 then null
		when communication_type='EMAIL' and length(substr(reverse(value),1,instr(reverse(value),'.')-1))<2 then null
		when communication_type='EMAIL' and value rlike '\\.comcn$' then REGEXP_REPLACE(value,'\\.comcn$','.com.cn') 
		else value
	end value
	,custom_extension
	,deleted
...
select 
	...
	,case
		when communication_type in ('MOB','TEL') then value
		when communication_type='EMAIL' and value rlike '^\\..*\\.$' then regexp_replace(substr(value,2,length(value)-2),'[^A-Za-z0-9-.@_]','')
        when communication_type='EMAIL' and value rlike '^\\.' then regexp_replace(substr(value,2),'[^A-Za-z0-9-.@_]','')
        when communication_type='EMAIL' and value rlike '\\.$' then regexp_replace(substr(value,1,length(value)-1),'[^A-Za-z0-9-.@_]','')
        else regexp_replace(value,'[^A-Za-z0-9-.@_]','') 
    end value
	,custom_extension
	,deleted
	,deleted1
...
select
	...
	consguid	cons_guid,
	case 
		when communicationtype in ('MOB','TEL') and regexp_replace(value,"[^+0-9]","") REGEXP '^\\+861[3-9][0-9]{9}$' THEN REPLACE(regexp_replace(value,"[^+0-9]",""),'+','')
		when communicationtype in ('MOB','TEL') and regexp_replace(value,"[^+0-9]","") REGEXP '^861[3-9][0-9]{9}$' THEN regexp_replace(value,"[^+0-9]","")
		when communicationtype in ('MOB','TEL') and regexp_replace(value,"[^+0-9]","") REGEXP '^1[3-9][0-9]{9}$' THEN CONCAT('86',regexp_replace(value,"[^+0-9]",""))
		when communicationtype in ('MOB','TEL') and regexp_replace(value,"[^+0-9]","") REGEXP '^01[3-9][0-9]{9}$' THEN CONCAT('86',substr(regexp_replace(value,"[^+0-9]",""),2))
		when communicationtype in ('MOB','TEL') and regexp_replace(value,"[^+0-9]","") REGEXP '^0861[3-9][0-9]{9}$' THEN substr(regexp_replace(value,"[^+0-9]",""),2)
		when communicationtype='EMAIL' then REGEXP_REPLACE(REGEXP_REPLACE(concat(substr(value,1,instr(value,'@')),REGEXP_REPLACE(substr(value,instr(value,'@')+1),'@','')),'[.]{2,}','.'),"(@\\.)|(\\.@)",'@')
	end value,
	customextension	custom_extension,
	case when `delete`=true then 1 when `delete`=false then 0 else null end deleted,
	case when delete_flag in(1,0) then delete_flag end deleted1,	
	...
	preferred	preferred
from source_gcdm.gcdm_bp_communication)a)b;